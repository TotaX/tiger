...
fun ppexpr pps e0 = 
	let
		...
		| ppe(VarExp(v, _)) =
			...
			add_string pps ")"; add_break pps (0, 0);
			end_block pps)
		| ppe(MinusOpExp (e,p)) = (add_string pps "MinusOpExp ("; ppe e; add_string pps ")")
		...
		| ppe(SeqExp(explist, _)) =
			...
			List.app (fn e => (ppe e;add_break pps (0, 0)))
                        (List.take (explist,(List.length explist)-1));
                        add_string pps "," ;ppe (List.last explist);
			add_string pps "])"; add_break pps(0, 0);
			end_block pps)
