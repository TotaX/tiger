...
fun linearize(stm0: stm) : stm list =
	let
		...
		| inmut(CALL(f, args)) =
			inmut f andalso List.all inmut args
		| inmut _ = false
		fun commute(EXP(CONST _), _) = true
		| commute(_, NAME _) = true
		| commute(_, CONST _) = true
		| commute(EXP(CALL(NAME "_checkIndexArray", x)), _) =
			List.all inmut x
		| commute(EXP(CALL(NAME "_checkNil", x)), _) =
			List.all inmut x
		| commute(EXP x, y) = inmut x orelse inmut y
		| commute _ = false
